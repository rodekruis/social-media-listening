...
def get_twitter(config):
    ...
    auth = tweepy.OAuthHandler(
        twitter_secrets["CONSUMER_KEY"], twitter_secrets["CONSUMER_SECRET"]
    )
    auth.set_access_token(
        twitter_secrets["ACCESS_TOKEN"], twitter_secrets["ACCESS_SECRET"]
    )
    api = tweepy.API(
    ...
    if config["track-twitter-users"]:
        ...
        for userID in tw_users:
            ...
            tweets = api.user_timeline(
                screen_name=userID, count=200, include_rts=False, tweet_mode="extended"
            )
            ...
            try:
                for page in tweepy.Cursor(
                    ...
                    try:
                        ...
                        logging.warning(
                            "Some error occurred, skipping page {0}:".format(n)
                        )
                        logging.warning(e)
                        pass
                    n += 1
                ...
                logging.warning(
                    "Some error occurred, skipping query {0}:".format(query)
                )
                logging.warning(e)
                pass
...
def get_youtube(config):
    ...
    ] = "1"  # Disable OAuthlib's HTTPS verification
...
def get_kobo(config):
    ...
    data_request = requests.get(
        f'https://kobonew.ifrc.org/api/v2/assets/{kobo_secrets["asset"]}/data.json',
        headers=headers,
    )
...
def get_facebook(config):
    ...
    graph = facebook.GraphAPI(access_token=facebook_secrets["token"], version="3.1")
    ...
    while True:
        for post in page_posts["data"]:
            stats = graph.get_object(
                id=post["id"], fields="message,shares,likes.summary(true)"
            )
            ...
            comments = {"data": []}
            try:
                comments = graph.get_object(id=post["id"], fields="comments")[
                    "comments"
                ]
            except KeyError:
                continue
