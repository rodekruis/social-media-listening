...
class Extract:
    ...
    def set_secrets(self, secrets):
        ...
        if self.source == "kobo":
            missing_secrets = secrets.check_secrets(
                ["KOBO_URL", "KOBO_ASSET", "KOBO_TOKEN"]
            )
        if missing_secrets:
            raise Exception(
                f"Missing secret(s) {', '.join(missing_secrets)} for source {self.source}"
            )
        else:
    ...
    def get_data_twitter(self):
        auth = tweepy.OAuthHandler(
            self.secrets["API_CONSUMER_KEY"], self.secrets["API_CONSUMER_SECRET"]
        )
        auth.set_access_token(
            self.secrets["API_ACCESS_TOKEN"], self.secrets["API_ACCESS_SECRET"]
        )
            ...
            for query in self.queries:
                try:
                    for page in tweepy.Cursor(
                        api.search_tweets, q=query, include_entities=True, count=100
                    ).pages():
                        try:
                            for tweet in page:
    ...
    def get_data_telegram(self):
        ...
        messages = telegram_client.loop.run_until_complete(
            self._scrape_telegram(telegram_client, self.channels, self.start_date)
        )
        ...
        for channel in telegram_channels:
            ...
            try:
                ...
                async for raw_message in telegram_client.iter_messages(
                    channel_entity, offset_date=start_date, reverse=True, wait_time=5
                ):
                    ...
                    if (
                        ...
                        and raw_message.post
                        and raw_message.replies
                    ):
                        ...
                        for post_id in replied_post_id:
                            try:
                                async for raw_reply in telegram_client.iter_messages(
                                    ...
                                    reply = self._from_telegram(
                                        raw_reply, channel, channel_members
                                    )
                                    if reply.text != "":
                                ...
                                logging.info(
                                    f"getting replies for {message.id} failed: {e}"
                                )
            except Exception as e:
    ...
    def _save_temp(self, df, dataname):
        ...
        if not os.path.exists("./temp"):
            os.mkdir("./temp")
        filename = (
            f"./temp/{self.source}_{dataname}_{self.start_date}_{self.end_date}.csv"
        )
