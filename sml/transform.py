...
class Transform:
    ...
    def set_secrets(self, secrets):
        ...
        if self.translator_name == "Microsoft":
            missing_secrets = secrets.check_secrets(
                ["MSCOGNITIVE_KEY", "MSCOGNITIVE_LOCATION"]
            )
        elif self.translator_name == "Google":
            missing_secrets = secrets.check_secrets(["GOOGLE_SERVICEACCOUNT"])
        if missing_secrets:
            raise Exception(
                f"Missing secret(s) {', '.join(missing_secrets)} for translator {self.translator_name}"
            )
        else:
    ...
    def set_translator(
        ...
        if model is None:
            ...
            raise ValueError(
                f"Translator {model} is not supported."
        ...
        if self.from_langs is None or self.to_langs is None:
            raise ValueError(
                f"Original and target language must be specified for translator"
            )
        ...
        for from_lang in self.from_langs:
            for to_lang in self.to_langs:
                if from_lang == to_lang:
                    raise ValueError(
                        f"Original and target language of translator must be different"
                    )
                ...
                if self.translator_name == "Opus-MT":
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Opus-MT does not support translations from {from_lang} to {to_lang}, please use Microsoft or Google"
                        )
                    ...
                    headers = {
                        "Ocp-Apim-Subscription-Key": self.secrets.get_secret(
                            "MSCOGNITIVE_KEY"
                        ),
                        "Ocp-Apim-Subscription-Region": self.secrets.get_secret(
                    ...
                    service_account_info = self.secrets.get_secret(
                    ...
                    translator["engine"] = google_translate.Client(
    ...
    def translate_text(self, text: str, translator):
        ...
        if self.translator_name == "Opus-MT":
            ...
            for retry in range(10):
                try:
                    ...
                    if translation_data["from_lang"] == "":
                        translation_data["from_lang"] = response[0]["detectedLanguage"][
                            "language"
                        ]
                    break
            ...
            for retry in range(10):
                try:
                    response = translator_engine.translate(
                        text, target_language=translator["to_lang"]
                    )
        ...
        if translation_data["text"] == text:
            logging.warning(
                "Translator returned identical message, check configuration."
            )
        return translation_data
    ...
    def set_classifier(
        ...
        if type is None:
            ...
            if type not in supported_classifier_types:
                raise ValueError(
                    f"Classifier of type {self.classifier_type} is not supported. "
        ...
        if task is None:
            ...
            if task not in supported_classifier_tasks:
                raise ValueError(
                    f"Classifier task {task} is not supported. "
        ...
        if self.classifier_task == "sentiment-analysis":
            ...
            if class_labels is not None:
                logging.warning(
                    f"Classifier task sentiment-analysis uses fixed class labels, "
                    f"overwriting {class_labels} with {self.class_labels}."
                )
        else:
            if class_labels is None:
                raise ValueError(
                    f"Class labels must be specified for classifier task {self.classifier_task}"
                )
            self.class_labels = class_labels
        ...
        if self.classifier_type == "huggingface-pipeline":
            try:
                self.classifier = pipeline(
                    self.classifier_task, model=self.classifier_model
                )
            except RepositoryNotFoundError:
                raise ValueError(
                    f"Transformer model {self.classifier_model} not found."
                )
            ...
            try:
                if not os.path.exists(
                    os.path.join(self.classifier_model, "config.json")
                ):
                    os.makedirs(self.classifier_model, exist_ok=True)
                    snapshot_download(
                        repo_id=self.classifier_model, local_dir=self.classifier_model
                    )
        ...
        if self.classifier_type == "huggingface-pipeline":
            if self.classifier_task == "sentiment-analysis":
                ...
                classification_data = [
                    ...
                    {
                        "topic": (
                            "POSITIVE" if result["label"] == "NEGATIVE" else "NEGATIVE"
                        ),
            ...
            if self.classifier_task == "zero-shot-classification":
                result = self.classifier(
                    sequences=text, candidate_labels=self.class_labels, multi_label=True
                )
                for label, score in zip(result["labels"], result["scores"]):
                    classification_data.append(
                        {"topic": label, "score": score, "agent": "model"}
                    )
        ...
        if self.classifier_type == "setfit":
            ...
            with open(os.path.join(self.classifier_model, "label_dict.json")) as infile:
                ...
                for ix, score in enumerate(scores):
                    classification_data.append(
                        {"topic": label_dict[str(ix)], "score": score, "agent": "model"}
                    )
    ...
    def classify_message(self, message: Message):
        if self.classifier_lang is None:
            ...
            text = next(
                x["text"]
                for x in message.translations
                if x["to_lang"] == self.classifier_lang
            )
        else:
            logging.warning(
    ...
    def set_anonymizer(self, name: str = None, lang: str = None):
        ...
        if self.anonymizer_name == "anonymization-app":
            ...
            raise ValueError(
                f"Anonymizer {name} is not supported. "
        ...
        for retry in range(10):
            try:
                request = requests.post(
                    self.anonymizer, json={"text": text, "model": "ensemble"}
                )
    ...
    def anonymize_message(self, message: Message):
        if self.anonymizer_lang is None:
            ...
            translation = next(
                x for x in message.translations if x["to_lang"] == self.classifier_lang
            )
            translation["text"] = self.anonymize_text(translation["text"])
            message.set_translation(translation)
        else:
            logging.warning(
                f"Anonymizer language is {self.anonymizer_lang}, but no corresponding translation was found"
        ...
        if type(locations_fields) is list:
            for loc_col in locations_fields:
                if loc_col not in gdf.columns:
                    logging.warning(
                        f"{loc_col} not in locations file {locations_file}, check config"
                    )
    ...
    def geolocate_message(self, message: Message):
        ...
        if "coordinates" not in message.info.keys():
            for locations_field in self.locations_fields:
                locations = [
                    loc.lower().strip()
                    for loc in self.locations_file[locations_field].values
                ]
                # exact string mathing; TBI NER
                loc_match = [
                    loc for loc in locations if loc in message.text.lower().strip()
                ]
                for loc in loc_match:
                    gdf_match = self.locations_file[
                        self.locations_file[locations_field] == loc
                    ]
            ...
            gdf_x = gpd.GeoDataFrame(
                {"geometry": message.info["coordinates"]}, index=[0], crs="EPSG:4326"
            )
    ...
    def set_wordfreq(self, wordfreq_lang, lemmatizer_model=None):
        ...
        if self.wordfreq_lang is None:
            raise ValueError(
                f"Original language must be specified for word frequencies"
            )
        if lemmatizer_model is None:
            ...
            if not spacy.util.is_package(spacy_model_name):
                try:
                    ...
                    # model not found
                    raise ValueError(
                        f"spaCy model {spacy_model_name} for word frequencies not found, "
        ...
        self.lemmatizer = spacy.load(
            spacy_model_name, disable=["tokenizer", "parser", "ner"]
        )

    def get_wordfreq(self, messages):
        if self.lemmatizer is None:
            raise RuntimeError(
                "Lemmatizer not initialized for word frequencies, use set_wordfreq()"
            )
        ...
        for text in text_list:
            lemmatized_text = " ".join(
                [token.lemma_ for token in self.lemmatizer(text)]
            )
        ...
        dict_word_freq = {
            ...
            for key, value in sorted(
                dict_word_freq.items(), key=lambda item: item[1], reverse=True
            )
        }
        # map to dataframe
        ...
        if self.translator_name is not None:
            if self.from_lang != self.wordfreq_lang:
                self.set_translator(
                    from_lang=self.wordfreq_lang,
                    to_lang=self.to_lang,
                    model=self.translator_name,
                )
            df_word_freq["Translation"] = df_word_freq["Word"].apply(
                lambda x: self.translate_message(x)["text"], axis=1
            )
